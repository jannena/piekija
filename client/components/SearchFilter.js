...
const SearchFilter = ({ query, filters, setQuery, __ }) => {
    ...
    const searchFilterTemplate = (title, type, data) => <>
        ...
        <div>
            <ul style={{ dispaly: "block", padding: 10 }}>
                <ShowMore data={data.filter(({ _id }) => _id).map(({ _id, value, count }) => <li style={listItemStyle} key={_id}>
                    <a href="javascript:void(0)" onClick={handleSearchFilterClick(type, value || _id)}>{_id} ({count})</a>
                </li>)} show={5} />
    ...
    return <div>
        <Expandable title={__("Filter search")} noPadding={true}>
            <Columns>
                <Column>
                    {searchFilterTemplate(__("Content types"), "contentType", filters.contentTypes.map(({ _id, count }) => ({ value: _id, _id: __(_id), count })))}
                </Column>
                <Column>
                ...
                <Column>
                    {searchFilterTemplate(__("Languages"), "languages", filters.languages.map(({ _id, count }) => ({ value: _id, _id: __(`lang-${_id}`, _id), count })))}
                </Column>
            </Columns>
        </Expandable>
    </div>;
